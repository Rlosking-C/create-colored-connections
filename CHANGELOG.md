...
- The in-game config screen is translated. It used to show the raw option names (`dyeConsumption`, `traceDistance`) and the English comments in every language, because NeoForge leaves an option's translation key empty unless the config sets one explicitly and Configured falls back to the path name when it finds none. Every option and category now carries a key, with the key itself naming the option and `<key>.tooltip` its description, in all nine languages.
